...
- **Linux-сервер без GUI (главный путь):** `install.sh` в корне репо — `git clone && cd CONTROLGUI && ./install.sh`. Ставит Node (apt/dnf/pacman/zypper/apk, фолбэк nvm), заводит `/usr/local/bin/controlgui`, спрашивает про systemd-автозапуск (через `cli.js service install`) и зовёт мастер `controlgui remote setup`. Данные при этом лежат РЯДОМ с кодом (`DATA_ROOT = ROOT`), поэтому после root-операций зовётся `ensure_data_owner` (chown на `data/`+`servers/`). Файл обязан быть `100755` и LF (`.gitattributes: *.sh eol=lf`).
- **Серверы из терминала (без веб-панели):** `controlgui server create|list|start|stop|restart|kill|cmd|info|rm|redownload` (`cli.js` → `cmdServer`). ВАЖНО: все операции идут через HTTP-API локальной панели (`apiCall` на 127.0.0.1:PORT), а НЕ через `lib/store` напрямую — store кеширует реестр в памяти (`let registry`), поэтому прямая запись в `servers.json` мимо живой панели ей не видна и была бы затёрта её следующим `save()`. Если панель не отвечает, `ensurePanel()` поднимает её фоном → писатель всегда один. CSRF-гард пропускает CLI, т.к. `sameOriginRequest` пускает запросы без заголовка `Origin`.
- **Настройка удалёнки из терминала:** `controlgui remote setup` (`cli.js` → `remoteSetup`) — мастер: пользователь → порт → включение → отпечаток серта. В CLI-процессе HTTPS-листенер НЕ поднимается (`remoteaccess.sync()` без `init()` — no-op), поэтому мастер сам выпускает серт, если его ещё нет, иначе отпечаток показать нечего. Промпты (`askLine`/`askHidden`) читают ОДИН общий буфер stdin — не подмешивать `readline`, иначе ввод из пайпа теряется.
- **Linux-пакеты:** `node linux/build-deb.js <версия>` → `.deb`; `node linux/build-tarball.js <версия>` → `.tar.gz` (оба — чистый Node, свои tar/ar/gzip). У тарбола СВОЙ `install.sh` (генерится в `build-tarball.js`, ставит в `/opt/controlgui`) — это НЕ корневой `install.sh`.
