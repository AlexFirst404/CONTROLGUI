...
function injectBackButton(html) {
  ...
  const script = '<style>#screen-list{display:none!important}</style>' +
    '<script>(function(){var L=' + target + ',M=location.hash.match(/^#server=([^/]+)(?:\\/.*)?$/),I=M&&M[1];' +
    'function valid(){var p="#server="+I,h=location.hash;return !!I&&(h===p||h.indexOf(p+"/")===0);}' +
    'function guard(){if(!valid()){location.replace(L);return false;}return true;}' +
    'if(!guard())return;' +
    ...
    'if(!a.some(function(s){return s&&s.id===id;}))location.replace(L);}).catch(function(){location.replace(L);});' +
    'var P=history.pushState,R=history.replaceState;' +
    'history.pushState=function(){var x=P.apply(this,arguments);guard();return x;};' +
    'history.replaceState=function(){var x=R.apply(this,arguments);guard();return x;};' +
    'addEventListener("hashchange",guard,true);addEventListener("popstate",guard,true);' +
    'document.addEventListener("click",function(e){' +
    'var b=e.target&&e.target.closest?e.target.closest("#btn-back"):null;' +
    'if(b){e.preventDefault();e.stopImmediatePropagation();location.href=L;}' +
    '},true);})();<\/script>';
