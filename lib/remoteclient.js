...
/* Список серверов, доступных пользователю на удалённой панели (для выбора при «Открыть»).
   Переиспользует сессию живого прокси, если он поднят (иначе разовый вход). */
async function serversOf(id) {
  const conn = getConn(id);
  if (!conn) return { error: 'Подключение не найдено' };
  const st = proxies.get(conn.id);
  const agent = (st && st.agent) ? st.agent : agentFor(conn);
  try {
    const cookie = (st && st.cookie) ? st.cookie : await login(conn, agent);
    const r = await upstreamJson(conn, agent, 'GET', '/api/servers', { Cookie: cookie });
    if (!st) agent.destroy();
    if (r.code !== 200) return { error: 'Не удалось получить список серверов (HTTP ' + r.code + ')' };
    let d;
    try { d = JSON.parse(r.body.toString('utf8')); } catch (e) { return { error: 'Некорректный ответ удалённой панели' }; }
    const servers = (d.servers || []).map((s) => ({
      id: s.id, name: s.name, type: s.type, version: s.version, status: s.status,
      players: Array.isArray(s.players) ? s.players.length : 0,
    }));
    return { ok: true, servers };
  } catch (e) { if (!st) agent.destroy(); return { error: e.message }; }
}
...
function agentFor(conn) {
  return new https.Agent({
    ...
    maxSockets: 16,
    maxFreeSockets: 4,
    // Последний освобождённый сокет с наименьшей вероятностью уже закрылся на
    // удалённой стороне; это уменьшает ECONNRESET после паузы между запросами.
    scheduling: 'lifo',
...
const RETRYABLE_NETWORK_CODES = new Set(['ECONNRESET', 'EPIPE', 'ETIMEDOUT', 'EAI_AGAIN', 'ECONNREFUSED']);
function isSafeRetryMethod(method) {
  return ['GET', 'HEAD', 'OPTIONS'].includes(String(method || '').toUpperCase());
}
/* Повтор допустим лишь до получения заголовков и только для чтения. POST/PUT/DELETE
   могли выполниться на удалённой машине до обрыва ответа — повтор создал бы дубль. */
function canRetryNetwork(method, error, responseStarted) {
  return !responseStarted && isSafeRetryMethod(method) &&
    RETRYABLE_NETWORK_CODES.has(String(error && error.code || '').toUpperCase());
}
...
/* socket timeout начинается только после соединения, поэтому отдельно ограничиваем
   DNS/TCP/TLS. На уже переиспользованном keep-alive сокете таймер не нужен. */
function armConnectTimeout(req) {
  let timer = null;
  const clear = () => { if (timer) { clearTimeout(timer); timer = null; } };
  req.once('socket', (socket) => {
    try { socket.setKeepAlive(true, 1000); } catch (e) { /* сокет мог уже закрыться */ }
    if (!socket.connecting) return;
    timer = setTimeout(() => req.destroy(timeoutError('таймаут подключения')), CONNECT_TIMEOUT);
...
function upstreamJsonOnce(conn, agent, method, upath, headers, body) {
  return new Promise((resolve, reject) => {
    ...
    const bad = (e) => {
      if (settled) return;
      settled = true;
      if (e && typeof e === 'object') e.cgResponseStarted = responseStarted;
      reject(e);
    };
    ...
    armConnectTimeout(r);
    r.setTimeout(15000, () => r.destroy(timeoutError('таймаут ответа')));
    r.on('error', bad);
...
async function upstreamJson(conn, agent, method, upath, headers, body, networkRetries) {
  const retries = networkRetries == null ? 1 : networkRetries;
  try {
    return await upstreamJsonOnce(conn, agent, method, upath, headers, body);
  } catch (e) {
    if (retries > 0 && canRetryNetwork(method, e, !!(e && e.cgResponseStarted))) {
      await delay(RETRY_DELAY);
      return upstreamJson(conn, agent, method, upath, headers, body, retries - 1);
    }
    throw e;
  }
}
...
/* Origin проверяем на ЛОКАЛЬНОЙ стороне до удаления заголовка. Иначе страница
   злоумышленника, угадав эфемерный порт, могла бы слепо отправить мутацию через
   прокси с его сохранённой авторизованной кукой удалённой панели. */
function sameOriginRequest(req) {
  if (req.method === 'GET' || req.method === 'HEAD' || req.method === 'OPTIONS') return true;
  const origin = req.headers.origin;
  if (!origin) return true; // CLI/нативный клиент, браузерный fetch Origin отправляет
  let parsed;
...
async function proxyRequest(st, req, res) {
  ...
  if (!sameOriginRequest(req)) {
    ...
    return res.end(JSON.stringify({ error: 'Запрос отклонён: чужой источник (cross-origin)' }));
  }
  ...
  const replayable = !hasBody || bodyBuf !== null; // chunked/большое → bodyBuf===null → без релогина
  await forward(st, req, res, bodyBuf, inject, {
    auth: replayable,
    network: isSafeRetryMethod(req.method),
  });
}
...
function forward(st, req, res, bodyBuf, inject, retries) {
  return new Promise((resolve) => {
    const retryState = retries || { auth: false, network: false };
    let responseStarted = false;
    ...
    armConnectTimeout(preq);
    let responseTimer = null;
    if (isSafeRetryMethod(req.method)) {
      responseTimer = setTimeout(() => preq.destroy(timeoutError('таймаут ответа удалённой панели')), RESPONSE_TIMEOUT);
    }
    const clearResponseTimer = () => {
      if (responseTimer) { clearTimeout(responseTimer); responseTimer = null; }
    };
    preq.once('response', clearResponseTimer);
    preq.once('close', clearResponseTimer);
    preq.on('error', async (e) => {
      clearResponseTimer();
      const localOpen = !res.destroyed && !res.writableEnded;
      if (localOpen && !res.headersSent && retryState.network &&
          canRetryNetwork(req.method, e, responseStarted)) {
        await delay(RETRY_DELAY);
        if (!res.destroyed && !res.writableEnded) {
          return resolve(await forward(st, req, res, bodyBuf, inject, {
            auth: retryState.auth,
            network: false,
          }));
        }
        return resolve();
      }
      if (!localOpen) return resolve();
      if (!res.headersSent) {
...
function injectBackButton(html) {
  // Отдельной кнопки в углу больше нет. Вместо неё перехватываем ВЕРХНЮЮ кнопку «Назад»
  // (#btn-back в шапке сервера) удалённой панели и уводим в локальный экран «Удалённые
  // панели» (#remote). Слушатель — на фазе перехвата (capture) + stopImmediatePropagation,
  // поэтому срабатывает раньше собственного обработчика приложения и подменяет его поведение.
  // Внутренние вкладки заменяют одну запись history: тогда возврат поднимает уже отрисованный
  // локальный #remote из истории, а не перезагружает index.html через видимый экран серверов.
  const target = JSON.stringify(LOCAL_URL + '#remote');
  const localBase = JSON.stringify(LOCAL_URL);
  const script = '<style>#screen-list,#screen-create,#screen-proxy,#screen-remote{display:none!important}</style>' +
    '<script>(function(){var L=' + target + ',B=' + localBase + ',M=location.hash.match(/^#server=([^/]+)(?:\\/.*)?$/),I=M&&M[1],Q=false;' +
    'function valid(){var p="#server="+I,h=location.hash;return !!I&&(h===p||h.indexOf(p+"/")===0);}' +
    'function cover(){var c=document.createElement("div");c.style.cssText="position:fixed;inset:0;z-index:2147483647;background:#0a0a0a";(document.documentElement||document).appendChild(c);}' +
    'function leave(e){if(Q)return false;Q=true;if(e){e.preventDefault();e.stopImmediatePropagation();}cover();' +
    'if(document.referrer.indexOf(B)===0&&history.length>1)history.back();else location.replace(L);return false;}' +
    'function guard(e){return valid()?true:leave(e);}' +
    'if(!guard())return;' +
    'fetch("/api/servers",{credentials:"same-origin"}).then(function(r){if(!r.ok)throw 0;return r.json();})' +
    '.then(function(d){var a=d&&d.servers||[],id;try{id=decodeURIComponent(I);}catch(e){id=I;}' +
    'if(!a.some(function(s){return s&&s.id===id;}))leave();}).catch(function(){leave();});' +
    'var R=history.replaceState;' +
    'history.pushState=function(){var x=R.apply(this,arguments);guard();return x;};' +
    'history.replaceState=function(){var x=R.apply(this,arguments);guard();return x;};' +
    'addEventListener("hashchange",function(e){guard(e);},true);addEventListener("popstate",function(e){guard(e);},true);' +
    'document.addEventListener("click",function(e){' +
    'var b=e.target&&e.target.closest?e.target.closest("#btn-back"):null;' +
    'if(b)leave(e);' +
    '},true);})();<\/script>';
  // Вставляем до скриптов приложения, чтобы недопустимый hash не успел открыть
  // удалённый список/настройки даже на один кадр. Fallback нужен для старых HTML.
  if (/<head(?:\s[^>]*)?>/i.test(html)) return html.replace(/<head(?:\s[^>]*)?>/i, (m) => m + script);
...
module.exports = {
  FILE, list, probe, save, remove, check, serversOf, open, stopProxy, stopAll,
  _test: { isSafeRetryMethod, canRetryNetwork },
};
