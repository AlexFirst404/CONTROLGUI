...
function injectBackButton(html) {
  // аккуратная «пилюля» в стиле кита (полупрозрачная, подсвечивается при наведении),
  // а не громоздкая кнопка — возврат из удалённой панели в локальный менеджер
  const btn = '<a href="' + LOCAL_URL + '" id="cg-remote-back" title="Вернуться к своим серверам">' +
    '<span style="display:inline-block;width:0;height:0;border-top:5px solid transparent;border-bottom:5px solid transparent;' +
    'border-right:7px solid currentColor;margin-right:2px"></span>Мои серверы</a>' +
    '<style>#cg-remote-back{position:fixed;left:14px;bottom:14px;z-index:2147483000;display:inline-flex;align-items:center;gap:8px;' +
    'padding:8px 14px;background:rgba(24,30,20,.82);color:#a7d585;border:1px solid #3c5230;border-radius:2px;' +
    'box-shadow:0 4px 14px rgba(0,0,0,.45);font:500 13px system-ui,-apple-system,Segoe UI,sans-serif;text-decoration:none;' +
    'backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px);transition:background .12s,color .12s,border-color .12s;opacity:.9}' +
    '#cg-remote-back:hover{background:rgba(40,58,34,.95);color:#c6ee9f;border-color:#5c8a3e;opacity:1}</style>';
