...
/* Локальная авторизация убрана (v1.4.3): личность = аккаунт центра (гейт при старте десктопа),
   локально всегда полный доступ владельцу. Удалённые запросы используют свой принципал
   (internalUserFor в lib/remote.js) и сюда не доходят. Каркас прав сохранён. */
