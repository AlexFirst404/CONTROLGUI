...
function setPassword(password) {
  ...
  // поколение пароля: инкремент делает недействительными ВСЕ ранее выданные сессии,
  // даже если пароль сменили из другого процесса (CLI) — панель проверяет epoch по файлу
  cfg.pwEpoch = (parseInt(cfg.pwEpoch, 10) || 0) + 1;
  saveCfg(cfg);
  sessions.clear(); // в этом же процессе — сразу
  return { ok: true };
}
function currentEpoch() { return parseInt(loadCfg().pwEpoch, 10) || 0; }
...
function createSession() {
  ...
  sessions.set(token, { expires: Date.now() + TTL_MS, epoch: currentEpoch() });
  return token;
}
...
function sessionFromReq(req) {
  ...
  // пароль сменили (в т.ч. из CLI) → epoch в файле вырос → старые сессии недействительны
  if ((s.epoch || 0) !== currentEpoch()) { sessions.delete(token); return null; }
  return { remote: true };
}
...
function disable() {
  const cfg = loadCfg();
  cfg.enabled = false;
  cfg.pwEpoch = (parseInt(cfg.pwEpoch, 10) || 0) + 1; // рвём все сессии (в т.ч. в процессе панели)
  saveCfg(cfg);
  sessions.clear();
  sync();
  return { ok: true };
