...
/* Долгие операции до появления записи сервера (например, копирование импорта)
   ещё не имеют serverId/state. Общий счётчик не даёт shutdown оборвать их. */
function beginGlobalMaintenance() {
  globalMaintenance++;
  let released = false;
  return () => {
    if (released) return;
    released = true;
    globalMaintenance = Math.max(0, globalMaintenance - 1);
  };
}
...
function getState(id) {
  ...
  if (!s) {
    s = {
      ...
      restoring: false, // каталог временно недоступен: идёт безопасное восстановление бэкапа
      backupCreating: false,
      backupProgress: null, // безопасный публичный снимок прогресса без путей и служебных имён
      activeMutations: 0, // server-specific записи, начатые до попытки restore
      startedAt: null,
      stopTimer: null,
      onExit: null,
      ...
      tps: null, tpsAt: 0,
      tpsAuto: false, tpsQueryAt: 0, tpsCommand: '', tpsCandidates: null,
      tpsDetectAt: 0, tpsCandidateIndex: 0, tpsWorkingCommand: '', tpsAwaitValues: false,
      tpsDimensionMin: null,
    };
    states.set(id, s);
  }
  return s;
...
function resetTpsState(s, clearValue) {
  if (clearValue) { s.tps = null; s.tpsAt = 0; }
  s.tpsAuto = false;
  s.tpsQueryAt = 0;
  s.tpsCommand = '';
  s.tpsCandidates = null;
  s.tpsDetectAt = 0;
  s.tpsCandidateIndex = 0;
  s.tpsWorkingCommand = '';
  s.tpsAwaitValues = false;
  s.tpsDimensionMin = null;
}

function restoringError(action) {
  const err = new Error('Нельзя ' + action + ': идёт восстановление бэкапа');
  err.status = 409;
  return err;
}

function assertNotRestoring(id, action) {
  if (getState(id).restoring) throw restoringError(action);
}

function isRestoring(id) {
  return !!getState(id).restoring;
}

/* Захват блокировки восстановления делаем синхронно до первого await: два
   одновременных запроса не смогут оба пройти проверку. Восстанавливать можно
   только полностью простаивающий сервер — переходные состояния и установка
   Java тоже способны позже запустить JVM и начать писать в заменяемый каталог. */
function beginRestore(id) {
  const s = getState(id);
  if (s.restoring) throw restoringError('начать ещё одно восстановление');
  if (s.backupCreating) {
    const err = new Error('Дождитесь завершения создания бэкапа'); err.status = 409; throw err;
  }
  if (s.download && !['done', 'error'].includes(s.download.phase)) {
    const err = new Error('Дождитесь завершения загрузки или установки ядра'); err.status = 409; throw err;
  }
  if (s.activeMutations > 0) {
    const err = new Error('Дождитесь завершения текущего изменения файлов или настроек сервера'); err.status = 409; throw err;
  }
  if (s.proc) {
    const err = new Error('Сначала остановите сервер'); err.status = 409; throw err;
  }
  if (orphanAlive(id)) {
    const err = new Error('Процесс сервера ещё работает — сначала завершите его'); err.status = 409; throw err;
  }
  if (s.javaInstalling) {
    const err = new Error('Дождитесь завершения установки Java или отмените запуск сервера'); err.status = 409; throw err;
  }
  if (s.status === 'starting') {
    const err = new Error('Дождитесь завершения запуска сервера'); err.status = 409; throw err;
  }
  if (s.status === 'stopping') {
    const err = new Error('Дождитесь полной остановки сервера'); err.status = 409; throw err;
  }
  if (s.status !== 'stopped' && s.status !== 'error') {
    const err = new Error('Сервер ещё не перешёл в состояние полного простоя'); err.status = 409; throw err;
  }
  if (s.download && !['done', 'error'].includes(s.download.phase)) {
    const err = new Error('Дождитесь завершения установки файлов сервера'); err.status = 409; throw err;
  }
  s.restoring = true;
  pushLine(id, '[ПАНЕЛЬ] Начинаю восстановление бэкапа — консоль временно недоступна.');
  for (const res of s.clients) {
    try { res.end(); } catch (e) { /* клиент уже отключился */ }
  }
  s.clients.clear();
}

/* Флаг снимается вызывающим кодом только через finally. После успешной замены
   каталога сбрасываем кэши, иначе UI показывал бы игроков и метрики из старого мира. */
function endRestore(id, successful) {
  const s = getState(id);
  if (successful) {
    s.historyMap = null;
    s.historyLoadError = null;
    s.players.clear();
    s.playersInfo.clear();
    s.stats = [];
    s.statsPrev = null;
    s.startedAt = null;
    resetTpsState(s, true);
    s.status = 'stopped';
  }
  s.restoring = false;
}
...
function pushLine(id, line) {
  ...
  for (const res of s.clients) {
    if (typeof res.cgConsoleAuthorize === 'function') {
      let allowed = false;
      try { allowed = !!res.cgConsoleAuthorize(); } catch (e) { /* сессия уже недоступна */ }
      if (!allowed) {
        s.clients.delete(res);
        try { res.end(); } catch (e) { /* сокет уже закрыт */ }
        continue;
      }
    }
...
function getHistoryMap(id) {
  ...
  if (!s.historyMap) {
    try {
      s.historyMap = JSON.parse(fs.readFileSync(historyFile(id), 'utf8')) || {};
      if (!s.historyMap || Array.isArray(s.historyMap) || typeof s.historyMap !== 'object') s.historyMap = {};
      s.historyLoadError = null;
    }
    catch (e) {
      s.historyMap = {};
      s.historyLoadError = e && e.code === 'ENOENT' ? null : e;
    }
  }
  return s.historyMap;
}

function writeHistoryMap(id, map) {
  ...
  let mode = 0o600;
  try { mode = fs.statSync(file).mode & 0o777; } catch (e) { /* новый файл с IP-историей */ }
  try {
    fs.writeFileSync(tmp, JSON.stringify(map, null, 1), { mode });
...
function recordJoin(id, p) {
  ...
  const entry = Object.assign({ name: p.name, uuid: null, firstJoinAt: null, lastJoinAt: null, ips: [] }, before || {});
  entry.ips = Array.isArray(entry.ips) ? entry.ips.slice() : [];
  map[key] = entry;
  ...
  // Синхронная атомарная запись исключает гонку «join пишет старую копию уже после
  // удаления игрока», из-за которой карточка иногда появлялась снова.
  try {
    writeHistoryMap(id, map);
    getState(id).historyLoadError = null;
  }
  catch (e) {
    if (before) map[key] = before; else delete map[key];
    console.error('[ПАНЕЛЬ] Не удалось сохранить историю игроков: ' + e.message);
  }
}
...
function removeHistory(id, name, uuids) {
  const state = getState(id);
  const map = getHistoryMap(id);
  if (state.historyLoadError) throw state.historyLoadError;
  const normName = stripAnsi(String(name || '')).trim().toLowerCase();
  const uuidKeys = new Set((uuids || []).map((value) => String(value || '').toLowerCase().replace(/-/g, ''))
    .filter((value) => /^[0-9a-f]{32}$/.test(value)));
  const removed = [];
  for (const [key, entry] of Object.entries(map)) {
    const entryName = stripAnsi(String(entry && entry.name || key)).trim().toLowerCase();
    const entryUuid = String(entry && entry.uuid || '').toLowerCase().replace(/-/g, '');
    if (entryName === normName || (uuidKeys.has(entryUuid) && /^[0-9a-f]{32}$/.test(entryUuid))) {
  ...
  try {
    ...
    for (const [key, entry] of removed) map[key] = entry;
    throw e;
  }
...
function forgetPlayer(id, name, uuids) {
  const s = getState(id);
  const norm = stripAnsi(String(name)).toLowerCase();
  const uuidKeys = new Set((uuids || []).map((value) => String(value || '').toLowerCase().replace(/-/g, ''))
    .filter((value) => /^[0-9a-f]{32}$/.test(value)));
  const forgottenNames = new Set([norm]);
  for (const key of Array.from(s.playersInfo.keys())) {
    const info = s.playersInfo.get(key);
    const infoUuid = String(info && info.uuid || '').toLowerCase().replace(/-/g, '');
    if (stripAnsi(key).toLowerCase() === norm || uuidKeys.has(infoUuid)) {
      forgottenNames.add(stripAnsi(key).toLowerCase());
      s.playersInfo.delete(key);
    }
  }
  for (const key of Array.from(s.players)) {
    if (forgottenNames.has(stripAnsi(key).toLowerCase())) s.players.delete(key);
  }
}
...
const UTF8_FLAGS = ['-Dfile.encoding=UTF-8', '-Dstdout.encoding=UTF-8', '-Dstderr.encoding=UTF-8'];

/* Подстановка переменных шаблона команды запуска (совместимо с Pterodactyl-стилем). */
function fillLaunchTemplate(tpl, server, memoryMb) {
  return String(tpl)
    .replace(/\{\{\s*SERVER_MEMORY\s*\}\}/g, String(memoryMb))
    .replace(/\{\{\s*SERVER_JARFILE\s*\}\}/g, server.jarFile || 'server.jar')
    .replace(/\{\{\s*SERVER_PORT\s*\}\}/g, String(server.port || 25565));
}

function buildJavaArgs(server) {
  const memoryMb = Math.max(512, parseInt(server.memoryMb, 10) || 2048);
  // кастомная команда запуска (пресеты Aikar и т.п.) с переменными {{SERVER_MEMORY}}/{{SERVER_JARFILE}}
  if (server.launchCmd && String(server.launchCmd).trim()) {
    let toks = fillLaunchTemplate(server.launchCmd, server, memoryMb).trim().split(/\s+/).filter(Boolean);
    if (toks.length && /^java(\.exe)?$/i.test(toks[0])) toks = toks.slice(1); // бинарь java берём из pickJava
    // гарантируем UTF-8, даже если пользователь их не указал (иначе кириллица ломается)
    if (!toks.some((t) => /-Dfile\.encoding/i.test(t))) toks = UTF8_FLAGS.concat(toks);
    return toks;
  }
  const spec = launchSpec(server);
  const base = ['-Xms512M', '-Xmx' + memoryMb + 'M'].concat(UTF8_FLAGS);
...
function start(server, retryAfterJava) {
  const s = getState(server.id);
  assertNotRestoring(server.id, 'запустить сервер');
  ...
  // Повторяем защиту при КАЖДОМ старте: backend, привязанный старой версией
  // панели, мог остаться в offline-mode на 0.0.0.0 и обходить вход через прокси.
  const behindProxy = store.all().some((p) => p.proxy && Array.isArray(p.proxyServers) &&
    p.proxyServers.some((b) => b && b.id === server.id));
  if (behindProxy) proxy.bindBackend(server);
  ...
  const onData = (key) => (data) => {
    ...
    while ((i = buffers[key].indexOf('\n')) >= 0) {
      ...
      if (line.trim()) {
        ...
        if (interceptTps(s, line)) continue;
  ...
  proc.on('exit', (code, signal) => {
    ...
    endStats(s);
    markAllOffline(s);
    resetTpsState(s, true); // сервер больше не работает — старое значение TPS недействительно
...
function stop(id) {
  const s = getState(id);
  assertNotRestoring(id, 'остановить сервер');
...
function kill(id) {
  const s = getState(id);
  assertNotRestoring(id, 'завершить процесс сервера');
...
function restart(server) {
  const s = getState(server.id);
  assertNotRestoring(server.id, 'перезапустить сервер');
  if (s.proc) {
    s.onExit = () => {
...
function sendCommand(id, command) {
  const s = getState(id);
  assertNotRestoring(id, 'отправить команду');
...
function pushStatPoint(s, cpuSec, ws, readB, writeB) {
  ...
  const point = {
    ...
    tps: s.tps != null && Date.now() - s.tpsAt < 30000 ? parseFloat(s.tps) : null,
  };
  s.stats.push(point);
...
function getStats(id) {
  const s = getState(id);
  assertNotRestoring(id, 'получить статистику сервера');
...
function attachConsole(id, res, authorize) {
  const s = getState(id);
  assertNotRestoring(id, 'открыть консоль');
  if (typeof authorize === 'function' && !authorize()) {
    const err = new Error('Доступ к консоли отозван'); err.status = 403; throw err;
  }
  res.writeHead(200, {
  ...
  // у пустой консоли writeHead без записи не уходит в сокет до heartbeat (25с) —
  // стартовый SSE-комментарий флашит заголовки сразу (браузеру и прокси удалёнки)
  res.write(': connected\n\n');
  for (const line of s.console) {
    res.write('data: ' + JSON.stringify(line) + '\n\n');
  }
  if (typeof authorize === 'function') res.cgConsoleAuthorize = authorize;
  s.clients.add(res);
    ...
    heartbeat = setInterval(() => {
      for (const state of states.values()) {
        for (const client of state.clients) {
          if (typeof client.cgConsoleAuthorize === 'function') {
            let allowed = false;
            try { allowed = !!client.cgConsoleAuthorize(); } catch (e) { /* сессия уже недоступна */ }
            if (!allowed) {
              state.clients.delete(client);
              try { client.end(); } catch (e) { /* сокет уже закрыт */ }
              continue;
            }
          }
...
function cpuModel() {
  try { return (os.cpus()[0] || {}).model || null; } catch (e) { return null; }
}

/* Внешний (публичный/WAN) IP — чтобы показать адрес для подключения друзей из интернета.
   Один исходящий запрос к публичному echo-сервису, результат кэшируется на 10 минут,
   при недоступности — null (UI откатывается на LAN-адрес). Без внешних зависимостей. */
let _extIp = { ip: null, at: 0, pending: null };
const EXT_IP_TTL = 10 * 60 * 1000;
function externalIp() {
  if (_extIp.ip && Date.now() - _extIp.at < EXT_IP_TTL) return Promise.resolve(_extIp.ip);
  if (_extIp.pending) return _extIp.pending;
  const https = require('https');
  const hosts = ['https://api.ipify.org', 'https://ipv4.icanhazip.com', 'https://ifconfig.me/ip'];
  const fetchOne = (url) => new Promise((resolve) => {
    let done = false;
    const fin = (v) => { if (!done) { done = true; resolve(v); } };
    try {
      const r = https.get(url, { headers: { 'User-Agent': 'curl/8' }, timeout: 4000 }, (res) => {
        let d = '';
        res.on('data', (c) => { d += c; if (d.length > 64) res.destroy(); });
        res.on('end', () => {
          const ip = d.trim();
          fin(/^\d{1,3}(\.\d{1,3}){3}$/.test(ip) ? ip : null);
        });
  ...
  _extIp.pending = (async () => {
    for (const h of hosts) {
      const ip = await fetchOne(h);
      if (ip) { _extIp = { ip, at: Date.now(), pending: null }; return ip; }
    }
    _extIp = { ip: null, at: Date.now(), pending: null }; // помним неудачу тоже 10 мин, не долбим
    return null;
  })();
  return _extIp.pending;
}

/* ---- TPS (тики/сек): Paper-гибриды отвечают на `tps`, Forge — на `forge tps`,
   NeoForge 1.21+ — на `neoforge tps`. Arclight снова использует Bukkit-команду.
   Ответ автоопроса скрываем, но введённую пользователем команду оставляем в консоли. */
const TPS_QUERY_TTL = 3500;

function advanceTpsCandidate(s) {
  const candidates = s.tpsCandidates || [];
  if (!candidates.length) return;
  const current = candidates.indexOf(s.tpsCommand);
  s.tpsCandidateIndex = ((current >= 0 ? current : s.tpsCandidateIndex) + 1) % candidates.length;
}

/* Единый признак для API/UI. Для custom-гибридов семейство определяется по jar,
   libraries и логу; отрицательный результат пересматривается раз в 15 секунд. */
function supportsTps(server) {
  if (!server) return false;
  const s = getState(server.id);
  const now = Date.now();
  if (!Array.isArray(s.tpsCandidates) || (!s.tpsCandidates.length && now - s.tpsDetectAt >= 15000)) {
    s.tpsCandidates = tps.commandsFor(server, serverDir(server.id));
    s.tpsDetectAt = now;
  }
  return s.tpsCandidates.length > 0;
}

function interceptTps(s, line) {
  const now = Date.now();
  const automatic = s.tpsAuto && now - s.tpsQueryAt <= TPS_QUERY_TTL;
  const parsed = tps.parseLine(line, automatic && s.tpsAwaitValues);
  if (parsed && parsed.header) {
    if (automatic) s.tpsAwaitValues = true;
    return automatic;
  }
  if (parsed && parsed.tps != null) {
    let value = parsed.tps;
    if (parsed.kind === 'dimension') {
      s.tpsDimensionMin = s.tpsDimensionMin == null ? value : Math.min(s.tpsDimensionMin, value);
      value = s.tpsDimensionMin;
    } else {
      s.tpsDimensionMin = null;
    }
    s.tps = String(value);
    s.tpsAt = now;
    if (automatic) {
      s.tpsWorkingCommand = s.tpsCommand;
      return true;
    }
    return false;
  }
  if (automatic && tps.isCommandError(line, s.tpsCommand)) {
    if (s.tpsWorkingCommand === s.tpsCommand) s.tpsWorkingCommand = '';
    s.tpsAwaitValues = false;
    s.tpsDimensionMin = null;
    return true;
  }
...
function startTpsPolling() {
  if (_tpsTimer) return;
  _tpsTimer = setInterval(() => {
    const now = Date.now();
    for (const [id, s] of states) {
      if (!s.proc || s.status !== 'running') continue;
      const srv = store.get(id);
      if (!srv) continue;
      if (!s.tpsCandidates || (!s.tpsCandidates.length && now - s.tpsDetectAt >= 15000)) {
        s.tpsCandidates = tps.commandsFor(srv, serverDir(id));
        s.tpsDetectAt = now;
      }
      if (!s.tpsCandidates.length) continue;
      // опрашиваем только когда кто-то смотрит (открыта консоль или недавно графики)
      if (s.clients.size === 0 && now - (s.lastStatsView || 0) > 9000) continue;
      if (s.tpsAuto && now - s.tpsQueryAt <= TPS_QUERY_TTL) continue;
      if (s.tpsAuto) {
        // Команда не дала распознаваемого ответа: в следующий раз пробуем
        // совместимый запасной вариант (полезно для импортированных гибридов).
        if (s.tpsAt < s.tpsQueryAt) {
          if (s.tpsWorkingCommand === s.tpsCommand) s.tpsWorkingCommand = '';
          advanceTpsCandidate(s);
        }
        s.tpsAuto = false;
      }
      if (s.tps != null && now - s.tpsAt >= 30000) s.tps = null;
      const command = s.tpsWorkingCommand || s.tpsCandidates[s.tpsCandidateIndex] || s.tpsCandidates[0];
      try {
        s.tpsAuto = true;
        s.tpsQueryAt = now;
        s.tpsCommand = command;
        s.tpsAwaitValues = false;
        s.tpsDimensionMin = null;
  ...
  if (_tpsTimer.unref) _tpsTimer.unref();
}
startTpsPolling();
...
/* Операции, которые нельзя обрывать выходом процесса: они атомарно меняют
   файлы либо готовят последующий запуск. Обычные живые JVM сюда не входят. */
function anyMaintenance() {
  if (globalMaintenance > 0) return true;
  const javaPhase = (javainstall.getState() || {}).phase;
  if (javaPhase === 'downloading' || javaPhase === 'extracting') return true;
  for (const s of states.values()) {
    if (s.restoring || s.backupCreating || s.activeMutations > 0 || s.javaInstalling ||
      (s.download && !['done', 'error'].includes(s.download.phase))) return true;
  }
  return false;
}
...
function anyActive() {
  ...
  if (javaPhase === 'downloading' || javaPhase === 'extracting') return true;
  for (const [id, s] of states) {
    if (s.proc || s.javaInstalling || s.restoring || s.backupCreating || s.activeMutations > 0 ||
      (s.download && !['done', 'error'].includes(s.download.phase)) ||
      s.status === 'starting' || s.status === 'stopping') return true;
...
module.exports = {
  ...
  beginRestore, endRestore, isRestoring,
  attachConsole, checkJava, lanAddresses, externalIp, systemMemoryMb, cpuCores, cpuModel, listJavas,
  isLaunchReady, jarPath, anyRunning, anyActive, anyMaintenance, beginGlobalMaintenance, stopAll, playersView,
