...
function pidIsJava(pid) {
  if (!IS_WIN) {
    try { return fs.readFileSync('/proc/' + pid + '/comm', 'utf8').trim().includes('java'); }
...
function listeningPorts() {
  ...
  try {
    if (IS_WIN) {
      ...
      const out = spawnSync('ss', ['-ltnp'], { encoding: 'utf8' }).stdout || '';
      for (const line of out.split('\n')) {
        const m = line.match(/:(\d+)\s+\S+\s+users:\(\("[^"]+",pid=(\d+)/);
        if (m && !map.has(+m[1])) map.set(+m[1], +m[2]);
      }
    }
  } catch (e) { /* утилита недоступна — пропускаем */ }
  return map;
}
...
function findForgeLaunch(dir) {
  // новые версии (1.17+): libraries/net/minecraftforge/forge/<ver>/<os>_args.txt
  const argsName = IS_WIN ? 'win_args.txt' : 'unix_args.txt';
  const forgeLib = path.join(dir, 'libraries', 'net', 'minecraftforge', 'forge');
  try {
    for (const ver of fs.readdirSync(forgeLib)) {
      const argsFile = path.join(forgeLib, ver, argsName);
...
/* приём сырых метрик {cpuSec, ws, readB, writeB} -> точка графика */
function ingestStats(s, raw) {
  if (!raw) {
    s.statsMisses++;
    return false;
  }
  ...
  s.statsPrev = { t: now, cpuSec: raw.cpuSec, readB: raw.readB, writeB: raw.writeB };
  if (!prev) return true;
  ...
  const point = {
    t: now,
    cpu: Math.max(0, Math.min(100, ((raw.cpuSec - prev.cpuSec) / dt) * 100 / cores)),
    ramMb: Math.round(raw.ws / 1048576),
    readBps: Math.max(0, (raw.readB - prev.readB) / dt),
    writeBps: Math.max(0, (raw.writeB - prev.writeB) / dt),
...
function retargetStatsPid(server, s) {
  if (s.statsMisses >= 2) {
    s.statsMisses = 0;
    const byPort = listeningPorts().get(Number(server.port));
...
/* Linux: метрики процесса из /proc — без внешних утилит */
function readLinuxStats(pid) {
  try {
    const stat = fs.readFileSync('/proc/' + pid + '/stat', 'utf8');
    const afterComm = stat.slice(stat.lastIndexOf(')') + 2).split(' ');
    const utime = parseInt(afterComm[11], 10) || 0; // поле 14
    const stime = parseInt(afterComm[12], 10) || 0; // поле 15
    const cpuSec = (utime + stime) / 100; // CLK_TCK=100 на практике везде

    const status = fs.readFileSync('/proc/' + pid + '/status', 'utf8');
    const rssMatch = status.match(/VmRSS:\s+(\d+)\s+kB/);
    const ws = rssMatch ? parseInt(rssMatch[1], 10) * 1024 : 0;

    let readB = 0;
    let writeB = 0;
    try {
      const io = fs.readFileSync('/proc/' + pid + '/io', 'utf8');
      readB = parseInt((io.match(/read_bytes:\s+(\d+)/) || [])[1], 10) || 0;
      writeB = parseInt((io.match(/write_bytes:\s+(\d+)/) || [])[1], 10) || 0;
    } catch (e) { /* нет прав на io — оставляем нули */ }
    return { cpuSec, ws, readB, writeB };
  } catch (e) {
    return null;
  }
}
...
function collectStats(server, s) {
  // java (JDK 21+) — лаунчер, порождающий реальную JVM отдельным процессом:
  // метрики снимаем с того, кто слушает порт сервера
  ...
  if (!IS_WIN) {
    if (!ingestStats(s, readLinuxStats(pid))) retargetStatsPid(server, s);
    return;
  }
  ...
  proc.on('exit', () => {
    ...
    const raw = parts.length === 4 ? {
      cpuSec: parseFloat(parts[0]) || 0,
      ws: parseInt(parts[1], 10) || 0,
      readB: parseInt(parts[2], 10) || 0,
      writeB: parseInt(parts[3], 10) || 0,
    } : null;
    if (!ingestStats(s, raw)) retargetStatsPid(server, s);
  });
}
