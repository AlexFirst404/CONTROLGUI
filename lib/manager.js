...
function getState(id) {
  ...
  if (!s) {
    s = {
      ...
      tps: null, tpsAt: 0, tpsAuto: false, // TPS Paper-ядер (авто-опрос `tps`)
    };
    states.set(id, s);
  }
  return s;
...
function start(server, retryAfterJava) {
  ...
  proc.on('exit', (code, signal) => {
    ...
    endStats(s);
    markAllOffline(s);
    s.tps = null; s.tpsAt = 0; s.tpsAuto = false; // сброс TPS — сервер больше не работает
...
function pushStatPoint(s, cpuSec, ws, readB, writeB) {
  ...
  const point = {
    ...
    tps: s.tps != null ? parseFloat(s.tps) : null, // TPS Paper-ядер (для графика; null у vanilla)
  };
  s.stats.push(point);
...
/* ---- TPS (тики/сек) для Paper-совместимых ядер: периодически шлём `tps` и ловим
   ответ, пряча его из консоли (как data get entity). Vanilla/Forge команду не знают. */
const PAPER_TPS = new Set(['paper', 'purpur', 'folia', 'mohist']);
function interceptTps(s, line) {
  const m = line.match(/TPS from last.*?:\s*[*]?(\d+(?:\.\d+)?)/i);
  if (!m) return false;
  s.tps = m[1];
  s.tpsAt = Date.now();
  if (s.tpsAuto) { s.tpsAuto = false; return true; } // наш авто-опрос — гасим строку
  return false; // пользователь сам ввёл /tps — показываем
}
let _tpsTimer = null;
function startTpsPolling() {
  ...
  _tpsTimer = setInterval(() => {
    for (const [id, s] of states) {
      ...
      const srv = store.get(id);
      if (!srv || !PAPER_TPS.has(srv.type)) continue;
      // опрашиваем только когда кто-то смотрит (открыта консоль или недавно графики)
      if (s.clients.size === 0 && Date.now() - (s.lastStatsView || 0) > 9000) continue;
      try { s.tpsAuto = true; s.proc.stdin.write('tps\n', 'utf8'); }
      catch (e) { s.tpsAuto = false; }
    }
  }, 7000);
