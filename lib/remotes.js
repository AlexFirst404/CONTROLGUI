...
/* Авто-регистрация: удалённая установка сама прислала свой адрес и пароль.
   Обновляем существующую запись или добавляем новую (помечаем auto). */
function upsertAuto(rawUrl, password, label) {
  const url = normalizeUrl(rawUrl);
  if (!url) return null;
  const items = load();
  const existing = items.find((r) => r.url === url);
  if (existing) {
    ...
    items.push({ url, password: password || '', label: label || '', auto: true });
  }
  save(items);
  return url;
}
...
/* Клиент саморегистрации: эта установка сообщает хабу свой адрес и пароль. */
async function postRegister(hubUrl, payload) {
  const res = await request('POST', String(hubUrl).replace(/\/+$/, '') + '/api/admin/register', {
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
    timeout: 8000,
  });
  if (res.status !== 200) throw new Error('хаб ответил HTTP ' + res.status);
  return true;
}

module.exports = { list, add, remove, upsertAuto, fetchAll, postRegister };
