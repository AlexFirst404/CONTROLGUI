...
function installJava(majorIn) {
  ...
  // Пользователь мог установить нужную Java уже после запуска панели. Перед
  // ручной загрузкой пересканируем ПК, чтобы не тратить трафик и место на второй JRE.
  javas.clearCache();
  const existing = javas.findJava(major);
  if (existing) {
    Object.assign(state, {
      phase: 'done', major, progress: 1, doneBytes: 0, totalBytes: 0,
      error: null, path: existing,
    });
    return state;
  }
