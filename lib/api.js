...
const { serverDir, DATA_ROOT, readLaunchMode, writeLaunchMode, readTrayMinimize, writeTrayMinimize } = require('./paths');
...
async function createServer(body, creator) {
  ...
  if (isImport) {
    const src = String(body.importPath || '');
    if (!src || !fs.existsSync(src) || !fs.statSync(src).isDirectory()) {
      throw fail(400, 'Папка существующего сервера не найдена');
    }
    if (path.resolve(src) === path.resolve(dir)) throw fail(400, 'Нельзя импортировать сам каталог панели');
