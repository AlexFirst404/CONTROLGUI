...
async function handle(req, res) {
  ...
  // удалённый доступ: статус и управление (менять — только с локальной панели,
  // чтобы удалённый гость не пересадил панель на другой порт/пароль и не отрезал владельца)
  if (parts[1] === 'remote-access') {
    if (method === 'GET') return json(res, 200, remoteaccess.status());
    if (method === 'POST') {
      requirePerm(req, 'settings.edit');
      if (isRemoteReq(req)) throw fail(403, 'Настройки удалённого доступа меняются только с локальной панели');
      const body = await readBody(req);
      const act = String(body.action || '');
      let r;
      if (act === 'set-password') r = remoteaccess.setPassword(body.password);
      else if (act === 'set-port') r = remoteaccess.setPort(body.port);
      else if (act === 'enable') r = remoteaccess.enable();
      ...
      return json(res, 200, Object.assign({ ok: true }, { status: remoteaccess.status() }));
    }
