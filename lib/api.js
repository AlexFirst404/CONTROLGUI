...
async function handle(req, res) {
  ...
  if (parts[1] === 'auth' && parts[2] === 'me' && method === 'GET') {
    return json(res, 200, {
      ...
      user: req.cgUser ? { username: req.cgUser.username, admin: !!(req.cgUser.perms && req.cgUser.perms.admin), perms: req.cgUser.perms } : null,
  ...
  if (parts[1] === 'users') {
    if (parts.length === 2) {
      ...
      if (method === 'POST') {
        ...
        const created = users.createUser(body.username, body.password, body.perms, firstEver ? true : !!body.admin);
    ...
    if (method === 'PUT') {
      const body = await readBody(req);
      return json(res, 200, { user: users.updateUser(uname, { password: body.password, perms: body.perms, admin: body.admin }) });
    }
    if (method === 'DELETE') {
  ...
  if (parts[1] === 'servers' && parts.length >= 3) {
    ...
    if (action === 'backups') {
      if (method === 'GET') {
        requirePerm(req, 'backups.manage');
    ...
    if (action === 'backup') {
      ...
      if (method === 'DELETE') { backups.deleteBackup(server.id, name); return json(res, 200, { ok: true }); }
      if (method === 'POST') { await backups.restoreBackup(server, name); return json(res, 200, { ok: true }); }
      throw fail(405, 'Метод не поддерживается');
    }
    ...
    if (action === 'player') {
      ...
      if (method === 'DELETE') { requirePerm(req, 'players.manage'); return json(res, 200, deletePlayerData(server, name)); }
      throw fail(405, 'Метод не поддерживается');
    }

    if (action === 'moderate' && method === 'POST') {
      requirePerm(req, 'players.manage');
      const body = await readBody(req);
    ...
    if (action === 'start') { requirePerm(req, 'server.power'); manager.start(server); return json(res, 200, serverView(server)); }
    if (action === 'stop') {
    ...
    if (action === 'kill') { requirePerm(req, 'server.power'); manager.kill(server.id); return json(res, 200, serverView(server)); }
    if (action === 'restart') { requirePerm(req, 'server.power'); manager.restart(server); return json(res, 200, serverView(server)); }
    if (action === 'download') {
      requirePerm(req, 'server.power');
      const s = manager.getState(server.id);
      if (s.proc) throw fail(409, 'Сначала остановите сервер');
