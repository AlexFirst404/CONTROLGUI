  ...
  if (parts[1] === 'servers' && parts.length >= 3) {
    ...
    if (action === 'backups') {
      if (method === 'GET') {
        ...
        // Сначала снимаем active-флаг, затем список. Если операция успеет
        // завершиться между чтениями, клиент увидит active и сделает ещё poll;
        // если active уже null, rename гарантированно предшествовал его очистке.
        const creating = users.hasPerm(req.cgUser, 'backups.create')
          ? backups.getCreationProgress(server.id) : null;
        return json(res, 200, {
          backups: backups.listBackups(server.id),
          // Текущий несжатый объём сервера нужен только тому, кто вправе
          // создавать снимок; delete/restore не получают лишнюю метрику.
          creating,
        });
      }
      if (method === 'POST') {
