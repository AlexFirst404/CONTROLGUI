...
function requireAllPerm(req, keys) {
  const u = req.cgUser;
  if (!keys.every((k) => users.hasPerm(u, k))) throw fail(403, 'Недостаточно прав для этого действия');
}
// запрос пришёл через прокси удалённого управления (синтетический принципал lib/remote.js)
function isRemoteReq(req) { return !!(req.cgUser && req.cgUser.remote); }
// удалённо без прав модерации — не отдаём IP игроков в списке/деталях сервера
function redactView(req, view) {
  if (!isRemoteReq(req) || !view) return view;
  const canIp = ['players.ban', 'players.kick', 'players.delete'].some((k) => users.hasPerm(req.cgUser, k));
  if (!canIp && Array.isArray(view.playersInfo)) view.playersInfo = view.playersInfo.map((p) => Object.assign({}, p, { ip: null }));
  return view;
}
...
// чувствительные/исполняемые цели записи: удалённо НЕ даём перезаписать (RCE через jar/launch + обход players.op/settings.edit)
// Нормализация сегмента имени под ловушки Windows: NTFS отбрасывает завершающие
// точки/пробелы, а «name::$DATA» / «name:stream» — это тот же файл (ADS). Приводим
// к «настоящему» имени, иначе имя-обманка обходит блок-лист и резолвится в защищённый файл.
function normalizeSeg(seg) {
  return String(seg).toLowerCase().split(':')[0].replace(/[ .]+$/, '');
}
function remoteWriteBlocked(target) {
  const parts = String(target).split(/[\\/]/).filter(Boolean);
  const name = normalizeSeg(parts[parts.length - 1] || '');
  // плагины/моды (.jar в plugins/ или mods/) — разрешаем удалённо: это легитимная установка
  const folder = normalizeSeg(parts[parts.length - 2] || '');
  if ((folder === 'plugins' || folder === 'mods') && /\.jar$/.test(name)) return false;
  // блокируем прямую замену запуска сервера (ядро/скрипты/args) и эскалацию прав (ops/whitelist/properties)
  return /\.(jar|sh|bat|cmd|exe|ps1)$/.test(name) ||
    name === 'eula.txt' || name === 'ops.json' || name === 'banned-players.json' ||
    name === 'banned-ips.json' || name === 'whitelist.json' || name === 'server.properties' ||
    /_args\.txt$/.test(name) || name === 'run.sh' || name === 'start.sh' || name === 'start.bat';
}
...
/* Карточка удалённого сервера для списка (детали грузятся при выборе через прокси к центру). */
function remoteServerView(s) {
  return {
    id: s.remoteLocalId, name: s.name, type: s.type || 'remote', version: s.version || '',
    port: 0, memoryMb: 0, cpuPercent: null, javaPath: '',
    plugins: false, mods: false, proxy: false, proxyServers: [],
    status: s.online ? (s.status || 'running') : 'offline',
    players: [], playersInfo: [], banned: [], ops: [], download: null,
    jarReady: true, startedAt: null, hasIcon: 0,
    remote: true, gid: s.globalId, online: !!s.online, ownerRole: s.role || null,
    owner: s.owner || null,
  };
}
...
async function createServer(body, creator) {
  ...
  store.add(server);
  autoEnableRemote(id); // авто-включение удалённого управления (лучшее усилие)
...
/* Авто-включить удалённое управление при создании сервера и привязать его к аккаунту
   центра (если в панель выполнен вход). Лучшее усилие — не валит создание, если центр
   недоступен или вход не выполнен; тогда удалёнку можно включить вручную в настройках. */
function autoEnableRemote(serverId) {
  setImmediate(async () => {
    try {
      const r = await remote.enable(serverId);
      if (r && r.linkCode && !r.claimed) {
        const cc = require('./centralclient');
        try { if (cc.state && cc.state().loggedIn) await cc.linkByCode(r.linkCode); } catch (e) { /* привяжется вручную */ }
      }
    } catch (e) { /* центр недоступен — удалёнку включат вручную позже */ }
  });
}
...
async function handle(req, res) {
  ...
  if (parts[1] === 'status' && method === 'GET') {
    ...
    const remote = isRemoteReq(req); // удалённо НЕ отдаём путь данных и LAN-адреса хоста
    return json(res, 200, {
      java,
  ...
  if (parts[1] === 'servers' && parts.length === 2) {
    if (method === 'GET') {
      ...
      const list = visible.map((s) => redactView(req, serverView(s)));
      // + удалённые серверы (добавленные по коду в десктопе) — показываем как обычные, с пометкой remote
      if (!isRemoteReq(req)) {
        try {
          // не дублируем свой же локальный сервер, у которого включён удалённый доступ:
          // он уже есть в списке как локальный — пропускаем его удалённую копию
          // (и по globalId локального сервера, и по совпадению remoteLocalId с локальным id)
          const localGids = new Set(visible.map((s) => s.remote && s.remote.globalId).filter(Boolean));
          const localIds = new Set(visible.map((s) => s.id));
          const remote = (await require('./centralclient').listRemote())
            .filter((s) => s.remoteLocalId && !localGids.has(s.globalId) && !localIds.has(s.remoteLocalId))
            .map(remoteServerView);
          for (const r of remote) list.push(r);
        } catch (e) { /* центр недоступен — только локальные */ }
      }
  ...
  if (parts[1] === 'servers' && parts.length >= 3) {
    ...
    if (!action) {
      if (method === 'GET') return json(res, 200, redactView(req, serverView(server)));
      if (method === 'DELETE') {
        ...
        // отвязать от центра (закрыть туннель + удалить запись), чтобы не остался «призрак»
        if (server.remote && (server.remote.enabled || server.remote.panelToken)) { try { remote.deregister(server.id); } catch (e) { /* */ } }
    ...
    if (action === 'proxy-link') {
      ...
      if (method === 'GET') {
        requirePerm(req, 'settings.edit');
        const proxies = store.all().filter((p) => proxy.isProxyType(p.type))
    ...
    // Удалённое управление: статус / вкл / выкл туннеля к центральному серверу.
    // Гейт СИЛЬНЕЕ, чем settings.edit: включение туннеля даёт ПОЛНЫЙ старт/стоп/рестарт
    // через интернет, а linkCode позволяет присвоить сервер. Поэтому требуем все силовые
    // права (start И stop) — иначе пользователь с одним settings.edit обошёл бы локальную
    // модель доступа (assigned = подмножество) и получил весь набор удалённо.
    if (action === 'remote') {
      if (parts[4] === 'regenerate' && method === 'POST') {
        requireAllPerm(req, ['server.start', 'server.stop']);
        try { const r = await remote.regenerate(server.id); return json(res, 200, Object.assign(remote.info(server.id), r)); }
        catch (e) { throw fail(502, e.message || 'Не удалось переиздать код'); }
      }
      if (method === 'GET') { requireAllPerm(req, ['server.start', 'server.stop']); return json(res, 200, remote.info(server.id)); }
      if (method === 'POST') {
        requireAllPerm(req, ['server.start', 'server.stop']);
        const body = await readBody(req);
        if (body.enabled) {
          // включение: поднять туннель и снять «скрытие» владельцем (если было)
          try { remote.ownerHide(server.id, false); const r = await remote.enable(server.id); return json(res, 200, Object.assign(remote.info(server.id), r)); }
          catch (e) { throw fail(502, e.message || 'Не удалось включить удалённое управление'); }
        }
        // выключение владельцем — «мягкое»: туннель остаётся для админа, владелец видит офлайн
        remote.ownerHide(server.id, true);
        return json(res, 200, remote.info(server.id));
      }
      throw fail(405, 'Метод не поддерживается');
    }
    ...
    if (action === 'plugins' || action === 'mods') {
      ...
      // провайдер каталога: modrinth (по умолчанию) или curseforge
      const providerOf = (v) => (v === 'curseforge' ? 'curseforge' : 'modrinth');

      if (sub === 'project' && method === 'GET') {
        requirePerm(req, 'files.read');
        const provider = providerOf(url.searchParams.get('provider'));
        const pid = url.searchParams.get('id') || '';
        const det = provider === 'curseforge'
          ? await curseforge.projectDetails(pid)
          : await modrinth.projectDetails(pid);
      ...
      if (sub === 'search' && method === 'GET') {
        ...
        if (provider === 'curseforge') {
          result = await curseforge.search({
        ...
        return json(res, 200, Object.assign({ mcVersion: server.version, loaders, provider }, result));
      }
      ...
      if (sub === 'install' && method === 'POST') {
        ...
        const ver = provider === 'curseforge'
          ? await curseforge.resolveVersion(String(body.projectId), server.version, isMod)
          : await modrinth.resolveVersion(String(body.projectId), server.version, loaders);
    ...
    if (action === 'player') {
      ...
      if (!name) throw fail(400, 'Не указано имя игрока');
      if (method === 'GET') {
        // данные игрока (инвентарь, IP) удалённо — только при каком-либо праве игроков/консоли
        if (isRemoteReq(req)) requireAnyPerm(req, ['players.kick', 'players.ban', 'players.op', 'players.whitelist', 'players.delete', 'console.view']);
        return json(res, 200, await playerDetails(server, name));
      }
    ...
    if (action === 'whitelist') {
      if (method === 'GET') {
        if (isRemoteReq(req)) requirePerm(req, 'players.whitelist');
        return json(res, 200, {
    ...
    if (action === 'file') {
      ...
      if (method === 'PUT') {
        ...
        // удалённо нельзя перезаписывать исполняемые/служебные файлы (RCE через jar/launch, обход players.op/settings.edit)
        if (isRemoteReq(req) && remoteWriteBlocked(target)) throw fail(403, 'Этот файл нельзя править удалённо — используйте вкладки «Настройки»/«Игроки».');
    ...
    if (action === 'file-upload' && method === 'PUT') {
      ...
      // удалённо нельзя заливать исполняемые/служебные файлы (RCE через jar/launch)
      if (isRemoteReq(req) && remoteWriteBlocked(rel)) throw fail(403, 'Этот файл нельзя загружать удалённо (исполняемые/служебные).');
    ...
    if (action === 'files-rename' && method === 'POST') {
      ...
      // удалённо нельзя переименовать НИ в, НИ из исполняемого/служебного имени —
      // иначе безобидный файл переименовывается в server.jar/run.sh/ops.json (RCE/эскалация)
      if (isRemoteReq(req) && (remoteWriteBlocked(from) || remoteWriteBlocked(to)))
        throw fail(403, 'Этот файл нельзя переименовывать удалённо (исполняемые/служебные).');
    ...
    if (action === 'files-extract' && method === 'POST') {
      ...
      const remoteReq = isRemoteReq(req);
      let written = 0;
      let skipped = 0;
      ...
      for (const entry of entries) {
        ...
        if (!name) continue;
        // удалённо: не даём распаковать исполняемые/служебные файлы (RCE/эскалация)
        if (remoteReq && remoteWriteBlocked(name)) { skipped++; continue; }
        let outAbs;
