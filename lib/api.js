...
async function handle(req, res) {
  ...
  if (parts[1] === 'servers' && parts.length >= 3) {
    ...
    // ---- плагины/моды (Modrinth): поиск, установка под версию сервера, список, удаление ----
    if (action === 'plugins' || action === 'mods') {
      ...
      if (!sub) {
        ...
        if (method === 'DELETE') {
          ...
          if (!/^[A-Za-z0-9_.\-]+\.jar(\.disabled)?$/i.test(name)) throw fail(400, 'Некорректное имя файла');
          try { fs.rmSync(path.join(contentDir, name), { force: true }); } catch (e) { /* нет файла */ }
          return json(res, 200, { ok: true });
        }
      }
      throw fail(405, 'Метод не поддерживается');
