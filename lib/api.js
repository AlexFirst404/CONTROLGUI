...
const TYPES = ['vanilla', 'paper', 'purpur', 'folia', 'mohist', 'forge'];
...
async function createServer(body) {
  ...
  if (!/^[\w.\-]+$/.test(version)) throw fail(400, 'Некорректная версия');
...
/* Сводка для админ-панели: по каждому серверу — ссылки для подключения и
   RCON-доступ (логин/пароль удалённого управления), плюс доступ к самой
   панели. Вся панель и так за паролем, так что показывать секреты можно. */
function adminOverview() {
  const ips = manager.lanAddresses();
  const hosts = ['localhost'].concat(ips);
  const panelPort = parseInt(process.env.PORT, 10) || 8400;

  const servers = store.all().map((server) => {
    const view = serverView(server);
    let p = {};
    try { p = props.parse(fs.readFileSync(path.join(serverDir(server.id), 'server.properties'), 'utf8')); }
    catch (e) { /* файла ещё нет */ }
    const rconEnabled = p['enable-rcon'] === 'true';
    return {
      id: server.id,
      name: server.name,
      type: server.type,
      ...
      links: hosts.map((h) => h + ':' + server.port),
      query: p['enable-query'] === 'true' ? { port: parseInt(p['query.port'], 10) || server.port } : null,
      rcon: {
        enabled: rconEnabled,
        port: parseInt(p['rcon.port'], 10) || 25575,
        password: rconEnabled ? (p['rcon.password'] || '') : '',
      },
    };
  });
  ...
  return {
    panel: {
      urls: hosts.map((h) => 'http://' + h + ':' + panelPort),
      login: 'admin',
      password: process.env.CONTROLGUI_PASSWORD || null,
      authEnabled: !!process.env.CONTROLGUI_PASSWORD,
    },
    servers,
  };
}
...
async function handle(req, res) {
  ...
  if (parts[1] === 'admin' && parts[2] === 'overview' && method === 'GET') {
    // includeRemotes=0 — нужна только локальная сводка (для опроса этой панели
    // другим дашбордом; иначе была бы рекурсия опросов)
    if (url.searchParams.get('includeRemotes') === '0') {
      return json(res, 200, adminOverview());
    }
    const remoteData = await remotes.fetchAll();
    return json(res, 200, Object.assign(adminOverview(), { remotes: remoteData }));
  }

  if (parts[1] === 'admin' && parts[2] === 'remotes') {
    if (method === 'GET') return json(res, 200, { remotes: remotes.list() });
    if (method === 'POST') {
      const body = await readBody(req);
      const added = remotes.add(body.url, body.password, body.label);
      return json(res, 200, { ok: true, url: added });
    }
    if (method === 'DELETE') {
      remotes.remove(url.searchParams.get('url') || '');
  ...
  if (parts[1] === 'servers' && parts.length === 2) {
    if (method === 'GET') return json(res, 200, { servers: store.all().map(serverView) });
    if (method === 'POST') return json(res, 201, await createServer(await readBody(req)));
    throw fail(405, 'Метод не поддерживается');
  }
  ...
  if (parts[1] === 'servers' && parts.length >= 3) {
    ...
    if (action === 'player') {
      ...
      if (method === 'DELETE') return json(res, 200, deletePlayerData(server, name));
      throw fail(405, 'Метод не поддерживается');
    }

    if (action === 'moderate' && method === 'POST') {
      const body = await readBody(req);
      return json(res, 200, await moderate(server, String(body.action || ''), String(body.name || '').trim()));
    }
    ...
    if (action === 'file') {
      const rel = url.searchParams.get('path') || '';
      if (method === 'GET') return json(res, 200, readTextFile(server.id, rel));
      if (method === 'PUT') {
        const body = await readBody(req);
    ...
    if (action === 'start') { manager.start(server); return json(res, 200, serverView(server)); }
    if (action === 'stop') {
      const s = manager.getState(server.id);
      if (!s.proc && manager.orphanAlive(server.id)) {
    ...
    if (action === 'kill') { manager.kill(server.id); return json(res, 200, serverView(server)); }
    if (action === 'restart') { manager.restart(server); return json(res, 200, serverView(server)); }
    if (action === 'download') {
      const s = manager.getState(server.id);
      if (s.proc) throw fail(409, 'Сначала остановите сервер');
      startDownload(server);
