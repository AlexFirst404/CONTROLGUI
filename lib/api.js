...
async function handle(req, res) {
  ...
  if (parts[1] === 'servers' && parts.length >= 3) {
    ...
    // ---- плагины (Modrinth): поиск, установка под версию сервера, список, удаление ----
    if (action === 'plugins') {
      if (!modrinth.supportsPlugins(server.type)) throw fail(400, 'Это ядро не поддерживает плагины');
      const sub = parts[4];
      const pluginsDir = path.join(serverDir(server.id), 'plugins');
      const loaders = modrinth.loadersFor(server.type);

      if (sub === 'search' && method === 'GET') {
        requirePerm(req, 'files.read');
        const hits = await modrinth.search(url.searchParams.get('q') || '', server.version, loaders);
        return json(res, 200, { hits, mcVersion: server.version, loaders });
      }

      if (sub === 'install' && method === 'POST') {
        requirePerm(req, 'files.upload');
        const body = await readBody(req);
        if (!body.projectId) throw fail(400, 'Не указан плагин');
        ...
        await dl.downloadFile(ver.url, path.join(pluginsDir, safeName));
        manager.pushLine(server.id, '[ПАНЕЛЬ] Установлен плагин ' + safeName + ' (' + ver.versionNumber + '). Перезапустите сервер, чтобы он загрузился.');
        return json(res, 200, { ok: true, file: safeName, version: ver.versionNumber });
      }
      ...
      if (!sub) {
        if (method === 'GET') {
          ...
          let installed = [];
          try {
            installed = fs.readdirSync(pluginsDir)
              .filter((f) => /\.jar(\.disabled)?$/i.test(f))
              .map((f) => {
                let size = 0; let mtime = 0;
                try { const st = fs.statSync(path.join(pluginsDir, f)); size = st.size; mtime = st.mtimeMs; } catch (e) { /* исчез */ }
        ...
        if (method === 'DELETE') {
          ...
          if (!/^[A-Za-z0-9_.\-]+\.jar(\.disabled)?$/i.test(name)) throw fail(400, 'Некорректное имя файла плагина');
          try { fs.rmSync(path.join(pluginsDir, name), { force: true }); } catch (e) { /* нет файла */ }
          return json(res, 200, { ok: true });
        }
      }
      throw fail(405, 'Метод не поддерживается');
    ...
    if (action === 'properties') {
      ...
      if (method === 'GET') {
        ...
        return json(res, 200, { properties, name: server.name, memoryMb: server.memoryMb, cpuPercent: server.cpuPercent == null ? null : server.cpuPercent });
      }
      if (method === 'PUT') {
        requirePerm(req, 'settings.edit');
        const body = await readBody(req);
