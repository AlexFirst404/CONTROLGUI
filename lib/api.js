...
function listDirs(p) {
  ...
  if (!p) {
    if (isWin) {
      ...
      return { path: '', parent: null, dirs: drives, isServer: false };
    }
    p = os.homedir() || '/';
  }
  ...
  return { path: abs, parent: (parent === abs ? '' : parent), dirs, jars, isServer };
}
...
async function handle(req, res) {
  ...
  if (parts[1] === 'servers' && parts.length === 2) {
    if (method === 'GET') {
      ...
      if (!isRemoteReq(req)) {
        try {
          const remote = (await require('./centralclient').listRemote()).filter((s) => s.remoteLocalId).map(remoteServerView);
  ...
  if (parts[1] === 'servers' && parts.length >= 3) {
    ...
    if (action === 'remote') {
      ...
      if (method === 'POST') {
        ...
        const body = await readBody(req);
        if (body.enabled) {
          try { const r = await remote.enable(server.id); return json(res, 200, Object.assign(remote.info(server.id), r)); }
          catch (e) { throw fail(502, e.message || 'Не удалось включить удалённое управление'); }
        }
        remote.disable(server.id);
    ...
    if (action === 'plugins' || action === 'mods') {
      ...
      if (sub === 'project' && method === 'GET') {
        ...
        const det = await modrinth.projectDetails(url.searchParams.get('id') || '');
      ...
      if (sub === 'search' && method === 'GET') {
        ...
        return json(res, 200, Object.assign({ mcVersion: server.version, loaders }, result));
      }
