...
async function createServer(body) {
  ...
  if (isImport) {
    ...
    // находим jar ядра (не installer)
    if (!fs.existsSync(path.join(dir, importedJar))) {
...
function listDirs(p) {
  ...
  for (const e of entries) {
    try {
      ...
      else if (e.isFile() && (/\.jar$/i.test(e.name) || e.name === 'server.properties' || e.name === 'eula.txt')) isServer = true;
    } catch (x) { /* */ }
  }
  dirs.sort((a, b) => a.localeCompare(b));
  const parent = path.dirname(abs);
  return { path: abs, parent: (parent === abs ? '' : parent), dirs, isServer };
}
