...
function serverView(server) {
  ...
  return {
    ...
    playersInfo: manager.playersView(server.id),
    download: s.download,
    jarReady,
...
function playerDetails(server, name) {
  ...
  // инвентарь и состояние из <world>/playerdata|players/data/<uuid>.dat
  let inventory = null;
  let details = {};
  let lastPlayed = null;
  try {
    ...
    inventory = (root.Inventory || []).map((item) => ({
      slot: item.Slot != null ? item.Slot : -1,
      id: String(item.id || '').replace(/^minecraft:/, ''),
      count: item.count != null ? item.count : (item.Count != null ? item.Count : 1),
    }));
    details = {
  ...
  return Object.assign({
    ...
    sessionStartedAt: live && live.online ? live.joinedAt : null,
...
async function handle(req, res) {
  ...
  if (parts[1] === 'servers' && parts.length >= 3) {
    ...
    if (action === 'player' && method === 'GET') {
      ...
      return json(res, 200, playerDetails(server, name));
    }
