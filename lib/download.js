...
const UA = 'CONTROLGUI-MinecraftPanel/' + VERSION + ' (open-source admin panel; +https://github.com/AlexFirst404/CONTROLGUI)';

const MOJANG_MANIFEST = 'https://piston-meta.mojang.com/mc/game/version_manifest_v2.json';
const PAPER_BASE = 'https://fill.papermc.io/v3/projects/';            // paper, folia (Fill API; старый v2 отключён 01.07.2026 — HTTP 410)
...
async function downloadFile(url, dest, onProgress) {
  ...
  await new Promise((resolve, reject) => {
    ...
    // при ошибке источника/записи res.pipe НЕ закрывает out (у него есть свой
    // обработчик error) — закрываем поток и удаляем недокачанный .part сами,
    // иначе копятся хвосты и течёт файловый дескриптор
    let failed = false;
    const fail = (e) => {
      if (failed) return; failed = true;
      res.destroy();  // рвём источник, иначе на ошибке записи (ENOSPC) тело качается «в никуда»
      out.destroy();
...
// ---- Paper / Folia / Velocity (PaperMC Fill v3) ----

function getPaperLikeVersions(project) {
  return cached('paperlike:' + project, async () => {
    // /versions отдаёт массив уже от новых к старым (в v2 было наоборот, приходилось reverse)
    const data = await fetchJson(PAPER_BASE + project + '/versions');
    return (data.versions || [])
      .filter((v) => !Array.isArray(v.builds) || v.builds.length) // версии без сборок скачать нельзя
      .map((v) => v.version && v.version.id)
      .filter(Boolean);
  });
}

async function resolvePaperLike(project, version) {
  // ответ — голый массив сборок, от новых к старым; каналы: ALPHA / BETA / STABLE / RECOMMENDED
  const builds = await fetchJson(PAPER_BASE + project + '/versions/' + encodeURIComponent(version) + '/builds');
  if (!Array.isArray(builds) || !builds.length) throw new Error('Нет сборок ' + project + ' для версии ' + version);
  const stable = builds.filter((b) => b.channel === 'STABLE' || b.channel === 'RECOMMENDED');
  const pool = stable.length ? stable : builds;
  // на порядок не полагаемся — берём сборку с максимальным id
  const build = pool.reduce((best, b) => (best && best.id >= b.id ? best : b), null);
  const dl = build.downloads && build.downloads['server:default'];
  if (!dl || !dl.url) throw new Error('У сборки ' + project + ' #' + build.id + ' нет файла server:default');
  // ссылка на fill-data.papermc.io уже готова в ответе — вручную собирать URL нельзя (формат может меняться)
  return dl.url;
}
