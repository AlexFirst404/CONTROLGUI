...
/* Принципал проксированного запроса: ТОЛЬКО с 127.0.0.1 + валидный внутренний секрет.
   Доступ ограничен ОДНИМ сервером (тем, чьему туннелю принадлежит запрос) и проброшенными
   правами. admin=false — панель-уровневые операции (юзеры) остаются недоступны удалённо. */
function internalUserFor(req) {
  if (!isLoopback(req.socket && req.socket.remoteAddress)) return null;
  if (req.headers[INTERNAL_HDR] !== INTERNAL_SECRET) return null;
  const perms = {};
  try { for (const k of JSON.parse(req.headers['x-cg-perms'] || '[]')) perms[String(k)] = true; } catch (e) { /* */ }
  const sid = String(req.headers['x-cg-server'] || '');
  return { username: String(req.headers['x-cg-actor'] || 'remote'), admin: false, perms, servers: sid ? [sid] : [] };
}
...
function postChunk(token, reqId, msg) { request('/agent/chunk', 'POST', Object.assign({ token, reqId }, msg)).catch(() => {}); }
...
function handleProxyHttp(serverId, cmd) {
  ...
  if (cmd.stream) {
    const lr = http.request(o, (resp) => {
      postChunk(token, cmd.reqId, { head: { status: resp.statusCode, headers: pickRespHeaders(resp.headers) } });
      let seq = 1;
      resp.on('data', (d) => postChunk(token, cmd.reqId, { seq: seq++, dataB64: d.toString('base64') }));
      resp.on('end', () => { postChunk(token, cmd.reqId, { done: true }); loopbackStreams.delete(cmd.reqId); });
      resp.on('error', () => { postChunk(token, cmd.reqId, { done: true }); loopbackStreams.delete(cmd.reqId); });
    });
    ...
    lr.setTimeout(14000, () => lr.destroy(new Error('loopback timeout')));
    if (cmd.bodyB64) lr.write(Buffer.from(cmd.bodyB64, 'base64'));
    lr.end();
  }
}
...
module.exports = { enable, disable, deregister, info, initAll, connect, statusOf, CENTRAL, internalUserFor };
