...
/* Вкл/выкл удалёнки у владельца — ЛИШЬ ВИЗУАЛ: туннель НЕ трогаем, удалёнка остаётся
   всегда реально включённой (админ всегда имеет доступ, статус на сайте — настоящий).
   Меняем только локальный флаг ownerHidden, который прячет/показывает код привязки в UI. */
