...
// Кадр SSE несёт проксируемый запрос целиком (вкл. bodyB64 правки файла). Центр доверенный
// (пиннинг серта), поэтому потолок высокий — но конечный, чтобы не копить бесконечно.
const MAX_SSE_BUF = 16 * 1024 * 1024;
...
function handleProxyHttp(serverId, cmd) {
  ...
  if (cmd.stream) {
    const lr = http.request(o, (resp) => {
      // ПОСЛЕДОВАТЕЛЬНАЯ отправка чанков + backpressure: пока чанк не доставлен на центр —
      // ставим loopback на паузу. Это сохраняет ПОРЯДОК и не копит память при медленном браузере.
      let chain = postChunk(token, cmd.reqId, { head: { status: resp.statusCode, headers: pickRespHeaders(resp.headers) } });
      resp.on('data', (d) => {
        resp.pause();
        chain = chain.then(() => postChunk(token, cmd.reqId, { dataB64: d.toString('base64') })).then(() => resp.resume());
      });
      resp.on('end', () => { chain.then(() => postChunk(token, cmd.reqId, { done: true })); loopbackStreams.delete(cmd.reqId); });
      resp.on('error', () => { chain.then(() => postChunk(token, cmd.reqId, { done: true })); loopbackStreams.delete(cmd.reqId); });
    });
