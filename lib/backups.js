...
const { ROOT, serverDir } = require('./paths');
...
function runTar(args, label) {
  return new Promise((resolve, reject) => {
    let proc;
    try { proc = spawn('tar', args, { windowsHide: true }); }
...
async function createBackup(server, label) {
  ...
  await runTar(['-czf', out, '-C', dir, '.'], 'Создание бэкапа');
  let size = 0;
  try { size = fs.statSync(out).size; } catch (e) { /* ? */ }
  manager.pushLine(id, '[ПАНЕЛЬ] Бэкап создан: ' + name);
  return { name, size };
}
