...
/* Секрет авто-привязки: удалённые установки регистрируются на этом хабе,
   предъявляя CONTROLGUI_HUB_SECRET (общий секрет, задаётся на хабе). */
function hubSecret() {
  return process.env.CONTROLGUI_HUB_SECRET || '';
}

function verifyHubSecret(input) {
