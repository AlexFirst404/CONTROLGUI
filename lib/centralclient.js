...
async function login(username, password) {
  ...
  saveAccount(false);
  let r;
  try { r = await cReq('/api/login', 'POST', { username, password }); } catch (e) { if (keepEp) saveAccount({ endpoint: keepEp }); return { error: 'Центральный сервер недоступен' }; }
...
async function register(username, password) {
  const prev = loadAccount(); const keepEp = prev && prev.endpoint ? prev.endpoint : null;
  let r;
  try { r = await cReq('/api/register', 'POST', { username, password }); } catch (e) { return { error: 'Центральный сервер недоступен' }; }
...
module.exports = {
  login, register, logout, state, me, rename, discordLinkInit, discordUnlink, refreshEndpoint,
