...
function migrateLegacyData() {
  try {
    ...
    if (!best) return;
    const mergeDir = (from, to, registryLast) => {
      if (!fs.existsSync(from)) return;
      if (!fs.existsSync(to)) fs.mkdirSync(to, { recursive: true });
      const names = fs.readdirSync(from).sort((a, b) => {
        if (!registryLast) return a.localeCompare(b);
        if (a === 'servers.json') return 1;
        if (b === 'servers.json') return -1;
      ...
      for (const name of names) {
        ...
        // Новые remote-настройки/секреты не перезаписываем, но каталоги миров и
        // бэкапов дотягиваем по отсутствующим элементам после частичного переноса.
        let sourceStat; let targetStat;
        try { sourceStat = fs.statSync(source); targetStat = fs.statSync(target); } catch (e) { continue; }
        if (sourceStat.isDirectory() && targetStat.isDirectory()) mergeDir(source, target, false);
      }
      try { if (fs.readdirSync(from).length === 0) fs.rmdirSync(from); } catch (e) { /* коллизии остаются как страховочная копия */ }
    };

    // Реестр — признак завершённого переноса, поэтому переносим его последним.
    // Если Windows/антивирус сорвёт перенос миров, следующий запуск увидит старый
    // servers.json и безопасно продолжит, а не сочтёт миграцию законченной.
    for (const sub of ['servers', 'backups', 'data']) {
      const from = path.join(best.dir, sub);
      const to = path.join(DATA_ROOT, sub);
      mergeDir(from, to, sub === 'data');
    }
...
if (!global.__controlguiSkipDataInit) {
  migrateLegacyData();
