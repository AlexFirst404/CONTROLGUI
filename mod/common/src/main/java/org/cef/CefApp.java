...
public class CefApp extends CefAppHandlerAdapter {
    ...
    private static final void setState(final CefAppState state) {
        ...
        // CONTROLGUI: раньше здесь был SwingUtilities.invokeLater — он поднимал
        // поток обработки событий AWT, что конфликтует с однопоточной моделью CEF
        // (особенно на macOS). Мы не используем appHandler_, поэтому зовём напрямую.
