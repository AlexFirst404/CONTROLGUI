...
Тело: `readBody(req)` (JSON, лимит 4МБ → 413). Загрузки: `receiveUpload` (стрим, 256МБ) пишет в уникальный соседний `.tmp` и только после полного приёма атомарно переименовывает, поэтому обрыв или превышение лимита не портит старый файл. Файлы: **всегда `safePath(serverId, rel)`** — кроме нормализации, отсечения `..` и `:` на Windows он проверяет `realpath` каждого существующего компонента и запрещает выход через symlink/junction. Ошибки: `fail(status,msg)` — с `.status` отдаётся текст как есть; без `.status` (500) удалённо обобщается в «Внутренняя ошибка сервера». `isRemoteReq(req)` скрывает чувствительное удалённо (путь `DATA_ROOT` и `lanIps` в `/api/status`, IP игрока без прав модерации, системный проводник).

## <a id="процессы"></a>5. Управление java-процессами — `lib/manager.js` (+ `download.js`, `javainstall.js`, `javas.js`, `coreinfo.js`)
Рантайм-состояние каждого сервера — в `Map states`, **НЕ персистится** (после рестарта панели всё `stopped`; связь с живым процессом — только через усыновление). `getState(id)` лениво создаёт запись: `proc`, `status` (stopped|starting|running|stopping|restoring|error), `console[]` (кольцо на 1000 строк), `clients` (Set SSE-ответов), `players/playersInfo`, `stats[]`, `orphanPid`, а также блокировки `restoring`, `backupCreating`, `activeMutations`.
...
**Консоль:** `stdout/stderr` → буфер → split `\n` → `stripAnsi` (гасит `\x1b[..m` и §-коды Paper/Purpur, иначе ломают регэкспы и имена игроков) → детект `Done (..s)!` → `status=running`; регэкспами ловятся версия, UUID, вход с IP, joined/left, достижения. **SSE `attachConsole`:** `text/event-stream`, сразу шлёт `: connected\n\n` (флашит заголовки — иначе пустая консоль висит до heartbeat через прокси удалёнки), отдаёт буфер, добавляет `res` в `clients`; `pushLine` рассылает всем. Один heartbeat-таймер (5с) на все серверы; перед строкой и heartbeat удалённый поток повторно проверяет живую сессию, доступ к серверу и `console.view`, поэтому отзыв прав закрывает уже открытую консоль.

**Restore-lock:** `beginRestore` синхронно захватывает сервер только в полном простое и только при нулевых `backupCreating/activeMutations`; существующие SSE закрываются. Пока `restoring`, запрещены консоль/команды/метрики, запуск/остановка/kill/restart, установка ядра, удаление и любые server-specific мутации. Перед восстановлением обязательно создаётся отдельный страховочный бэкап; архив проверяется на ссылки/спецфайлы и лимиты, распаковывается в соседний staging-каталог и лишь затем заменяет рабочий каталог. Реестровый порт сохраняется в staged `server.properties`. Мультисерверные операции прокси атомарно учитывают все затрагиваемые runtime-состояния. `/api/quit` и сигналы ОС ждут восстановления, бэкапов, мутаций, установки Java, фоновой загрузки/установки ядра и полного выхода штатно остановленных JVM, чтобы завершение процесса не оборвало атомарную замену каталога, незаконченный `.part`/installer или сохранение большого мира.
...
**Усыновление сирот `adoptOrphans`:** при старте панели ищет живой java-процесс каждого сервера (по сохранённому store `pid` или по слушаемому `server.port`). Найденный → `orphanPid`, консоль недоступна, можно только «Убить». `anyActive` учитывает сирот, `javaInstalling` и активную загрузку/установку ядра для гейта `/api/quit`.
...
- **Host/Origin-guard:** прокси слушает строго `127.0.0.1`, пропускает только loopback `remoteAddress` + Host ∈ {127.0.0.1, localhost, [::1]}; мутирующий браузерный запрос до stripping заголовков проходит тот же same-origin принцип, что основная панель.
...
- В GET `/` до скриптов приложения инъектится навигационный guard (форсит `accept-encoding: identity`, буфер HTML лимитирован 8МБ). Он фиксирует выбранный `#server=<id>`, проверяет id через `/api/servers`, скрывает удалённый `#screen-list` и при back/hash/history либо ошибке возвращает в локальный `#remote`. Корень удалённой панели никогда не открывается.

## <a id="хранилище"></a>8. Хранилище и пути — `lib/store.js`, `lib/paths.js`
- `store.js` — реестр `data/servers.json` формата `{servers:[...]}`, кэш в памяти. API: `all/get/add/update(id,patch)/updateMany/remove`. Запись — уникальный `.tmp` + атомарный `rename`; при ошибке диск и кэш откатываются вместе. `get()` возвращает **живой** объект из кэша (прямая мутация видна, но без `save()` не персистится — всегда через методы store).
- `paths.js` — `DATA_ROOT` = `env CONTROLGUI_DATA` или рядом с кодом; `DATA_DIR`, `SERVERS_DIR`, `REGISTRY_FILE`. `serverDir(id)` — для импортированных «на месте» серверов возвращает `server.dir` (внешняя папка — её содержимое при удалении сервера **не трогать**), иначе `SERVERS_DIR/id`. `launchMode`(app/browser) и `tray-minimize` — в фиксированном per-user каталоге (`%LOCALAPPDATA%\CONTROLGUI` / `~/.local/share/controlgui`). `migrateLegacyData()` рекурсивно переносит старые `servers/`, `backups/` и `data/`, а `servers.json` — последним: частичный перенос на Windows можно безопасно продолжить, и свежие настройки удалёнки не перетираются.
- Любое изменение `server.properties` проходит одну backend-транзакцию: порт строго `1024..65535`, уникален среди серверов, файл и реестр откатываются вместе. У backend, привязанного к прокси, порт нельзя менять до отвязки. Импорт «на месте» сравнивает канонические `realpath`, не принимает пустой путь и не позволяет зарегистрировать одну папку дважды.
...
- **Карточки:** `.srv-grid` держит до трёх карточек в строке и балансирует хвост по числу элементов: 4 → 2+2, 5 → 3+2, 7 → 3+2+2 и т.д.; медиазапросы не меняют это распределение.
- **Restore UX:** статус `restoring` блокирует lifecycle-кнопки, ввод/вынос консоли, создание/удаление бэкапов и конфликтующие действия; история консоли остаётся видимой, после завершения активная вкладка перечитывается.
- **Настройки приложения:** полноценный экран `#screen-app-settings` с вкладками удалённого доступа, приложения и интерфейса. Локальные prefs лежат в `controlgui-settings`; консольные флаги зеркалируются в legacy-ключи для отдельного окна консоли.
- **Консоль:** автопрокрутка, WARN-фильтр, перенос/размер текста и видимость каждой карточки графиков управляются на вкладке «Интерфейс»; token-aware подсказки дополняют команды, подкоманды и имена известных игроков.
- **Файлы/плагины/игроки:** распаковка доступна только для `.zip`; удаление плагина отдельно выбирает JAR либо JAR+безопасно найденную папку данных; player details показывает смерти, убийства игроков и мобов из world stats; адреса и локальный путь на вкладке «Информация» — read-only поля с кнопкой копирования.
- **Уведомления:** `showToast(msg, type)` (`'ok'` = зелёный, иначе красный), история с action-кнопками в верхней панели; доступный релиз добавляет уведомление и один раз предлагает модалку на главной. `confirmDialog`/`promptDialog` (Promise), `guard(fn)`.
...
- **`cli.js`** (`controlgui`): `serve`/`start`/`stop`/`status` (демон через pid-файл + HTTP; `cmdStop` шлёт `/api/quit` с `x-cg-stop-servers`, а при недоступном API **не убивает PID автоматически**, потому что устаревший pid-файл может указывать на чужой процесс), `remote user/enable/disable/port/cert-reset` (делегирует в remoteaccess/users), `service install/uninstall` (systemd от `SUDO_USER`, дом из `getent`, абсолютный `CONTROLGUI_DATA`), `connect` (клиент-режим GUI), `tui`. `askHidden` — ввод пароля без эха; в не-TTY читает РОВНО одну строку (не до EOF — иначе второй промпт пароля зависнет).
...
| `updates.js` / `version.js` | Проверка GitHub Release, выбор платформенного артефакта, потоковая загрузка с лимитом и SHA-256, запуск локального установщика / единая runtime-версия |

## <a id="сборка"></a>12. Сборка и релиз
См. раздел «Сборка и релиз» в [`../AGENTS.md`](../AGENTS.md) — тело панели, платформы, **список ~10 мест версии**, git-флоу. Ключевое: `.deb`/`.tar.gz` — чистый Node; AppImage/pkg — CI `workflow_dispatch`; Windows — локально (`panel.zip` → `dotnet publish` → ISCC), `.iss`/`.csproj` не в git. В Windows-обёртке `config.json` разделяет `PanelPath` (код) и `DataPath` (серверы/бэкапы/настройки): переход на новый встроенный `panel.zip` не должен сбрасывать прежний корень данных или копировать десятки гигабайт между дисками. Деинсталлятор сохраняет пользовательские данные. `CONTROLGUI.Browser.csproj` застрял на 1.3.0 (запасная обёртка, не синхронизируется). `Installer.cs` — мёртвый код.
...
data/
  ...
  update-state.json       — версия обновления, отложенная пользователем
  updates/                — проверенные установочные файлы новых версий
