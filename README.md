...
Каждая установка CONTROLGUI самостоятельна и знает только свои серверы. Свести
их в один дашборд можно двумя способами.

**Авто-привязка (push, без ввода пароля).** На центральном дашборде (хабе)
задайте секрет `CONTROLGUI_HUB_SECRET=любая_строка`. При установке сервера
передайте скрипту адрес хаба и тот же секрет:
```bash
sudo CONTROLGUI_HUB=http://ХАБ:8400 CONTROLGUI_HUB_SECRET=секрет bash scripts/install-linux.sh
```
Сервер сам зарегистрируется в дашборде (адрес + свой пароль) на старте и далее
раз в 2 минуты. Требование: хаб должен быть **доступен с этого сервера**
(публичный адрес; localhost-дашборд на домашнем ПК серверу недоступен).

**Вручную (pull).** В разделе **«Удалённые панели»** добавьте адрес установки
(`http://IP:8400`) и её пароль — дашборд сам опросит её и покажет её серверы.
Требование: установка доступна **с машины дашборда**. Если пишет «неверный
пароль», возьмите точный пароль на сервере: `sudo cat /opt/controlgui/data/panel.env`.

Адреса/пароли удалёнок хранятся в `data/admin-remotes.json` (в .gitignore).
